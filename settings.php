...
/*
	...
	https://github.com/uwejacobs/wp-pexels-free-stock-photos.git
	Author: Uwe Jacobs, Raaj Trambadia (https://raajtram.com)
*/
...
function media_pexels_fsp_images_tab() {
	...
	<div style="padding:10px 15px 25px">
       ...
       <p>2. Pick your photo, click on it <u>once</u> to download. It will be added to your Image Library</p>
       <p>3. Use the image as desired i.e. as a Featured Image or in your posts/page via the Gutenberg/Classic Editor</p>
       <p><strong>NOTE: Clicking on an image will download and add it to your Media Library. Do <u>not</u> click on an image to preview  a larger version!</strong></p>
       </div>
	...
	<script>
	   ...
	   function escapejs(s){return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,"\\'");}
		 /* set vars */
	   var post_id = <?= absint($_REQUEST['post_id'] ?? 0) ?>,
	       per_page = 30,
	   ...
	   function render_px_results(q, p, data){
		...
		var url = new URL(v.src.large2x);
		var h = Number(url.searchParams.get("h")) * 2;
		var w = Number(url.searchParams.get("w")) * 2;
		if (h > 0 || w > 0) {
			v.width = w;
			v.height = h;
		}
		s += '<div class="item upload" data-url="'+v.src.large2x+'" data-user="'+v.photographer+'" data-src-page="'+v.url+'" data-w="'+v.width+'" data-h="'+v.height+'" data-alt="'+v.alt+'"><img src="'+v.src.medium+'"><div class="download"><img src="<?= plugin_dir_url(__FILE__) . 'img/download.svg' ?>"><div>'+(v.width)+'×'+(v.height)+'<br><a href="'+v.url+'/" rel="noopener nofollow" target="_blank"">'+v.photographer+' at Pexels</a></div></div></div>';
	     });
	 ...
	 image_src_page: escape(jQuery(this).data('src-page')),
...
function pexels_fsp_images_settings_page() { ?>
    ...
    <p>
        Photos provided by <a href="https://pexels.com/?utm_source=wordpress-plugin&utm_medium=settings-page" target="_blank"><img src="<?= plugin_dir_url(__FILE__).'img/pexels-logo.png' ?>" style="margin:0 3px;position:relative;top:4px" width="80"></a>.
    </p>
    </div>
<?php }
...
if (isset($_POST['pexels_fsp_upload'])) {
	...
	/* get image file */
	$response = wp_remote_get(esc_url_raw($_POST['image_url']));
	...
	/* Name the file. */
	$post_url = strtok($_POST['image_url'], '?');
	$path_info = pathinfo(esc_url_raw($post_url));
